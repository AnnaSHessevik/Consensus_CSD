    ...
    try:
        ...
        if sys.argv[3] == "Y":
            ...
            if sys.argv[4] == "scatter":
                ...
                plt.xlim([-1, 1])
                plt.ylim([-1, 1])
                plt.xlabel("Estimated combined Spearman rank correlation coefficient \nusing weighted untransformed values", fontweight='bold')
                plt.ylabel("Estimated combined Spearman rank correlation coefficient \nusing Fisher's Z transformed values", fontweight='bold')
                plt.show()
            ...
            #Heatmap
                ...
                plt.xlabel("Estimated combined Spearman rank correlation \ncoefficient using weighted untransformed values", fontweight='bold')
                plt.ylabel("Estimated combined Spearman rank correlation \ncoefficient using Fisher's Z transformed values", fontweight='bold')
                plt.text(-0.9, 0.8, r'$r_s$ = $%.4f$' % (spearman[0]), fontsize=12)
                #plt.show()
