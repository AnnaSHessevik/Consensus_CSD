...
def make_plots(var1, var2, col, x_lab, y_lab, fig_name, nr, var1_specific, var2_specific, col_specific):    
    ...
    if ("Jaccard" in y_lab): 
        ...
        order=[1,0]
        plt.legend([handles[i] for i in order], [labels[i] for i in order], loc="lower right", bbox_to_anchor=(1.01,0.03), prop={'size': 17})
...
if __name__ == '__main__':
    ...
    plt.rcParams["mathtext.default"]='bf' #make mathematical expression in plot (here: x axis) bold
    make_plots(degrees_fisher, jaccard_list, "indigo", 	"$k_{transformed}$", "Jaccard index", "degree_vs_jaccard_CSD_fisher_v2.png", 1, degrees_f_specific, jaccard_specific, "mediumorchid")
    make_plots(degrees_arithmetic, jaccard_list, "forestgreen", "$k_{untransformed}$", "Jaccard index", "degree_vs_jaccard_CSD_arithmetic_v2.png", 2, degrees_a_specific, jaccard_specific, "lime")
    make_plots(degrees_arithmetic, degrees_fisher, "firebrick", "$k_{untransformed}$", "$k_{transformed}$", "degree_vs_degree_CSD_v2.png", 3, degrees_a_specific, degrees_f_specific, "lightcoral")
