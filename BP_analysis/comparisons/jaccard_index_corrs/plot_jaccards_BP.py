...
def generate_plot(infiles):
    ...
    ax.set_xscale('log')
    plt.ylim((0,1))
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.17), ncol=4, fontsize = "medium", edgecolor = "gray")
    plt.xlim((1,10000000))
	#Save figure
    plt.savefig("jaccards_arithmetic_ctrl.png", dpi=500, bbox_inches='tight')
    plt.show()
...
if __name__ == '__main__':
    ...
    for nr in geo:
        filename = "jaccard_arithmetic_vs_spearman_ctrl_" + nr + ".txt" #remember to change fisher and BP depending on wanted plot
        infiles.append(open(filename))
    infiles.append(open("jaccard_ctrl.txt")) #Fisher's Z vs arithmetic   
