...
def generate_plot(infiles):
    ...
    ax.set_ylabel('Jaccard index', fontweight='bold', fontsize=12)
    ax.set_xlabel("Number of investigated gene pairs", fontweight='bold', fontsize=12)
    ax.set_xscale('log')
    plt.ylim((0,1))
    plt.xlim(1, 1000000)
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.3), ncol=3, fontsize = "10", edgecolor = "gray")
    #Save figure
