...
def make_heatmap(correct_corr):
    ...
    plt.xlabel("Estimated combined Spearman rank correlation\ncoefficient using Fisher's Z transformed values", weight='bold', fontsize=18)
    plt.ylabel("Spearman rank correlation\ncoefficient from reference data set", weight='bold', fontsize=18)
    #plt.show()
    plt.text(-0.9, 0.8, r'$r_s$ = $%.4f$' % (spearman_fisher[0]), fontsize=22)
    ...
    cb = plt.colorbar()
    cb.set_label('Number of entries')
    plt.xlabel("Estimated combined Spearman rank correlation\ncoefficient using weighted untransformed values", weight='bold',fontsize=18)
    plt.ylabel("Spearman rank correlation\ncoefficient from reference data set", weight='bold', fontsize=18)
    plt.text(-0.9, 0.8, r'$r_s$ = $%.4f$' % (spearman_arithmetic[0]), fontsize=22)
    plt.plot([-1, 1], [-1,1],linestyle='dashed', color = 'black', linewidth=2)
