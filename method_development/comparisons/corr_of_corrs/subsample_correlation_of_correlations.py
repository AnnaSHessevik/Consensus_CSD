...
as calculated for an entire data set relative to Spearman rank correlations from a subsample of the data set.
    ...
    try:
        ...
        if sys.argv[2] == "Y":
            ...
            if sys.argv[3] == "scatter":
                ...
                plt.xlim([-1, 1])
                plt.ylim([-1, 1])
                plt.xlabel("Spearman rank correlation coefficient from subsample", fontweight='bold')
                plt.ylabel("Spearman rank correlation coefficient\nfrom reference data set", fontweight='bold')
                #plt.show()
        ...
        #Heatmap
            if sys.argv[3] == "heatmap":
                ...
                plt.xlabel("Spearman rank correlation coefficient\nfrom subgroup", fontweight='bold', fontsize=18)
                plt.ylabel("Spearman rank correlation\ncoefficient from reference data set", fontweight='bold', fontsize=18)
                #plt.show()
                plt.text(-0.9, 0.8, r'$r_s$ = $%.4f$' % (spearman[0]), fontsize=22)
