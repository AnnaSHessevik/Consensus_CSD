...
Fishers Z transformation and weighted untransformed averages.
...
if __name__ == '__main__':
    if len(sys.argv) != 5:
        ...
        #Save the number of the input file to make the output file more reader friendly
        infiles_number = []
        for name in infiles_name:
            name_split = name.split('.') #seperate .txt from rest of file names
            last_two = name_split[0][-2:] #get the last 2 characters (potential numbers)
            infiles_number.append(re.sub('[^0-9]','',last_two))
